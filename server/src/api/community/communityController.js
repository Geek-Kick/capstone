...
exports.recommend = async (req, res) => {
  const userId = req.userId;
  const schema = joi.recommendJoi;
  const req_body = req.body;
  try {
    await schema.validateAsync(req_body);
    const result = await service.postRecommend(userId, req_body);
...
exports.commentSelection = async (req, res) => {
  const userId = req.userId;
  const schema = joi.selectionJoi;
  const req_body = req.body;
  try {
    await schema.validateAsync(req_body);
...
exports.getMyScrap = async (req, res) => {
  const userId = req.userId;
  try {
    const result = await service.getMyScrap(userId);
    return res.status(200).send(result);
  } catch (e) {
    console.log(`controller error \n ${e}`);
    return res.status(400).json({ success: false, message: e.message });
