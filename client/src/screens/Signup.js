...
const DEFAULT_PHOTO = 'https://firebasestorage.googleapis.com/v0/b/rn-chat-fb183.appspot.com/o/profile-circle.png?alt=media';
const Signup = ({ navigation }) => {
    ...
    useEffect(() => {
        setDisabled(!(nickname && password && passwordConfirm && email && !errorMessage))
    ...
    useEffect(() => {
        if (refDidMount.current) {
            ...
            else if (password.length < 6) { error = '비밀번호가 6자리 이상이어야 합니다.' }
            else if (password !== passwordConfirm) { error = '비밀번호가 다릅니다!' }
            else if (!nickname) { error = '닉네임을 입력하세요'; }
            else { error = ''; }
